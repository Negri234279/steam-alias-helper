...
function setSub(text) {
    ...
    if (n) n.sub.textContent = text
}

function setTarget(text) {
    const n = overlayNodes()
    if (n) n.target.textContent = text
}
...
function findClickableByText(regex, root = document) {
    const candidates = [
        ...$all('a', root),
        ...$all('button', root),
...
async function trySetNickname(alias) {
    ensureOverlay()
    ...
    let nicknameBtn = null
    for (const r of dropdownRoots) {
        nicknameBtn =
            findClickableByText(/set nickname|Añadir alias|alias|nickname/i, r) ||
            findClickableByText(/nickname/i, r)
    ...
    const input =
        $("input[type='text']", modal) || $("input[type='search']", modal) || $('textarea', modal)
    ...
    const saveBtn =
        findClickableByText(/save|ok|set|guardar|aceptar/i, modal) ||
